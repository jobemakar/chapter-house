...
The TypeScript port passed `npm run check`: strict `tsc --noEmit`, deterministic TypeScript simulation/render tests, and an esbuild production build. The simulation test covers sealed reservoir/edges, canonical and straight-down winning routes, all three canteens, mouth/body fairness, post-delivery drainage, capped dummy, close bedrock and ten reset cycles. The renderer test covers threshold, joined/disconnected fluid and edge safety. The unchanged third-party Box2D.js/WASM remains vendor code.

Run with `npm run check`.

## Browser smoke test

With the local server running, `npm run test:browser` checks image loading, a complete mouse route, victory, all collectibles, reset, hint state, a 320-pixel layout, real synthesized touch input, and browser/network errors. The 2026-09-19 production-bundle smoke passed with no browser errors. Screenshots are written to `tests/artifacts/` and intentionally ignored by Git.
