...
export class WishboneCamera {
  ...
  private clamp() {
    ...
    this.x = Math.max(halfW, Math.min(this.width - halfW, this.x));
    this.y = Math.max(halfH, Math.min(this.height - halfH, this.y));
  }
  ...
  update(
    ...
    this.lastMode = mode;
    if (this.manual || this.zoom === 1) return;
    const target =
      mode === "flight"
