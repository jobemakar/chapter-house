...
const API_ROOT = "/api/contraption";
...
/** Local editor-only JSON file service. The plugin is deliberately serve-only. */
export function createLevelStorePlugin(
  options: LevelStoreOptions = {},
): Plugin {
  const handler = createLevelStoreHandler(options);
  return {
    name: "contraption-local-level-store",
    apply: "serve",
...
export function createLevelStoreHandler(
  options: LevelStoreOptions = {},
...
async function handleRequest(
  ...
  try {
    ...
    if (request.method === "PUT" && pathname === `${API_ROOT}/index`) {
      requireJson(request);
      const index = parseClientValue(
        await readJsonBody(request),
        parseLevelIndex,
      );
      await validateIncludedFiles(root, index);
      await atomicWrite(root, "index.json", index);
      sendJson(response, 200, { ok: true });
      return;
    }
    const match = /^\/api\/contraption\/levels\/([^/]+)$/.exec(pathname);
    if (match && (request.method === "GET" || request.method === "PUT")) {
      ...
      if (request.method === "GET") {
        ...
        if (file.level.id !== id)
          throw new HttpError(
            400,
            `Document id ${file.level.id} does not match filename ${id}`,
          );
        sendJson(response, 200, file);
        return;
      }
      requireJson(request);
      const file = parseClientValue(
        await readJsonBody(request),
        parseLevelFile,
      );
      if (file.level.id !== id)
        throw new HttpError(
          400,
          `URL id ${id} does not match document id ${file.level.id}`,
        );
    ...
    throw new HttpError(
      405,
      "Method or Contraption API route is not supported",
    );
  } catch (error) {
    const status =
...
async function prepareRoot(configuredRoot: string): Promise<string> {
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory())
    throw new HttpError(
      500,
      "Configured levels directory must be a real directory",
    );
  const realRoot = await fs.realpath(configuredRoot);
  if (path.resolve(realRoot) !== configuredRoot)
    throw new HttpError(
      500,
      "Configured levels directory cannot traverse a symbolic link",
    );
  return realRoot;
}
...
async function listLevels(
  root: string,
): Promise<{ levels: Array<{ id: string; name: string; issues: string[] }> }> {
  const entries = await fs.readdir(root, { withFileTypes: true });
  const filenames = entries
    .filter(
      (entry) => entry.name.endsWith(".json") && entry.name !== "index.json",
    )
    .map((entry) => entry.name)
    .sort((a, b) => a.localeCompare(b));
  const levels: Array<{ id: string; name: string; issues: string[] }> = [];
  for (const filename of filenames) {
    const id = filename.slice(0, -5);
    if (!isSafeLevelId(id)) continue;
    try {
      const file = await readLevel(root, filename);
      if (file.level.id !== id) {
        levels.push({
          id,
          name: file.level.name || id,
          issues: [
            `Document id ${file.level.id} does not match filename ${id}`,
          ],
        });
      ...
      levels.push({ id, name: file.level.name, issues: levelIssues(file) });
    } catch (error) {
      levels.push({ id, name: id, issues: [errorMessage(error)] });
    }
  }
  return { levels };
}
...
async function validateIncludedFiles(
  root: string,
  index: LevelIndex,
): Promise<void> {
  for (const id of index.levels) {
    ...
    if (file.level.id !== id)
      throw new HttpError(
        400,
        `Cannot include ${id}: document id is ${file.level.id}`,
      );
...
async function readJsonFile(root: string, filename: string): Promise<unknown> {
  ...
  if (stat.size > MAX_LEVEL_PAYLOAD_BYTES)
    throw new HttpError(413, `${filename} exceeds the 512 KB limit`);
...
async function atomicWrite(
  ...
  if (Buffer.byteLength(serialized) > MAX_LEVEL_PAYLOAD_BYTES)
    throw new HttpError(413, "Serialized document exceeds the 512 KB limit");
  try {
...
function validateLocalRequest(
  ...
  if (
    ...
    throw new HttpError(
      403,
      "Contraption level storage is available only on the local host",
    );
  ...
  if (
    ...
    throw new HttpError(
      403,
      "Cross-origin level storage requests are not allowed",
    );
}
