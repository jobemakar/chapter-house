...
import { ContraptionAudio } from "./audio";
import { ContraptionEngine, hit } from "./engine";
import {
  clamp,
  clone,
  kit,
...
class ActiveClock {
  ...
  interact() {
    this.action = this.now;
  }
  step(dt: number, live: boolean, observed: boolean) {
    this.now += dt;
    if (
      live &&
      (this.now - this.action < 5 || (observed && this.now - this.action < 9))
    )
      this.total += dt;
    return this.total;
  }
  suspend() {
    this.action = -Infinity;
  }
}
const deviceText: Record<PartType, string> = {
  ramp: "Tilt the ramp to guide corn.",
  belt: "Arrows show carrying direction.",
  spring: "A tiny tilt makes a big leap.",
...
export class ContraptionGame implements GameSession {
  private progress: ContraptionProgress;
  private engine: ContraptionEngine;
  private canvas: HTMLCanvasElement;
  private renderer = new ContraptionRenderer();
  private audio: ContraptionAudio;
  ...
  private placing: PartType | null = null;
  ...
  private activity: ActiveClock;
  private showedHint = false;
  constructor(
    ...
    host.addEventListener("change", this.changeInput, {
  ...
  private chosen() {
    return this.engine.parts.find((p) => p.id === this.selected);
  }
  ...
  private remaining(type: PartType) {
    return (
      this.budget(type) -
      this.engine.parts.filter((p) => !p.locked && p.type === type).length
    );
  }
  private sync() {
    const l = this.engineLevel(),
    ...
    this.q("[data-u=selected]").textContent = this.placing
      ? `Place a ${kit[this.placing].name}`
      : p
        ? (p.locked ? "Bolted · " : "") + kit[p.type].name
        : "Tap a movable part";
    this.q("[data-u=tip]").textContent = p
      ? deviceText[p.type]
      : "Bolted fixtures stay put. Drag the warm dashed devices.";
    this.q("[data-u=status]").textContent = this.paused
      ? "Tinkering time"
      : document.hidden
        ? "Machine suspended"
        : "Machine running · spills return safely.";
    ...
    if (!kitBox.children.length)
      for (const t of Object.keys(kit) as PartType[]) {
        ...
        kitBox.append(b);
        this.renderer.icon(b.querySelector("canvas")!, t);
      }
    for (const b of kitBox.querySelectorAll<HTMLButtonElement>("button")) {
      const t = b.dataset.a!.slice(4) as PartType;
      b.hidden = this.budget(t) === 0;
      b.disabled = this.remaining(t) <= 0;
      b.setAttribute("aria-pressed", String(this.placing === t));
      b.querySelector("small")!.textContent =
        `${Math.max(0, this.remaining(t))} / ${this.budget(t)} available`;
    }
    for (const a of ["left", "right", "flip", "duplicate", "remove"]) {
    ...
    this.q("[data-u=reward]").textContent =
      this.progress.delivered >= 20
        ...
        : this.progress.delivered >= 5
          ? "✓ Spring ornament earned · 20 deliveries earns the mini machine"
  ...
  private note(s: string) {
    const n = this.q("[data-u=toast]");
    n.textContent = s;
    n.classList.add("show");
    clearTimeout(this.toastTimer);
    this.toastTimer = window.setTimeout(() => n.classList.remove("show"), 3500);
  }
  private invalidate() {
  ...
  private click = (ev: MouseEvent) => {
    const b = (ev.target as Element).closest<HTMLButtonElement>(
      "button[data-a]",
    );
    if (!b) return;
    const a = b.dataset.a!;
    ...
    if (a.startsWith("kit:")) {
      this.placing =
        this.placing === a.slice(4) ? null : (a.slice(4) as PartType);
      this.selected = null;
      this.sync();
      return;
    }
    ...
    if (a === "starter") {
      this.replace(
        clone(this.engineLevel().initial),
        "Starter arrangement restored.",
      );
      return;
    }
    if (a === "clear") {
      this.replace(
        this.engine.parts.filter((p) => p.locked),
        "Movable parts returned to the toolbox.",
      );
      return;
    }
    if (a === "undo") {
    ...
    if (a === "right") this.edit((p) => (p.angle += Math.PI / 12));
    if (a === "flip")
      this.edit((p) =>
        p.type === "belt" ? (p.flip *= -1) : (p.angle += Math.PI),
      );
    if (a === "duplicate") {
      const p = this.chosen();
      if (p) this.add(p.type, p.x + 35, p.y + 40);
    }
    if (a === "remove") {
      const p = this.chosen();
  ...
  private changeInput = (ev: Event) => {
    const t = ev.target;
    if (t === this.q("[data-u=level]")) {
      this.change(Number((t as HTMLSelectElement).value));
      this.activity.interact();
    } else if (t === this.q("[data-u=slow]")) {
      this.progress.slow = (t as HTMLInputElement).checked;
      this.persist();
    } else if (t === this.q("[data-u=trails]")) {
      this.progress.trails = (t as HTMLInputElement).checked;
      this.persist();
    }
  };
  private point(e: PointerEvent): Point {
    ...
    return {
      x: ((e.clientX - r.left) * 1100) / r.width,
  ...
  private down = (e: PointerEvent) => {
    ...
    if (this.placing) {
      this.add(this.placing, q.x, q.y);
      return;
    }
    const p = [...this.engine.parts]
      .reverse()
    ...
    if (p && (!p.locked || p.type === "lever")) {
      this.drag = {
        ...
        dx: p.x - q.x,
        dy: p.y - q.y,
        before: clone(this.engine.parts),
        moved: false,
      };
  ...
  private key = (e: KeyboardEvent) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.code === "KeyZ") {
      e.preventDefault();
      this.q<HTMLButtonElement>("[data-a=undo]").click();
    }
  };
  private visibility = () => {
    if (document.hidden) {
    ...
    } else this.audio.setMuted(this.progress.mute);
  };
  private frame = (now: number) => {
    if (this.disposed) return;
    const dt = Math.min(0.06, (now - this.last) / 1000 || 0);
    this.last = now;
    const live = !this.paused && !document.hidden;
    let observed = false;
    if (live) {
      this.accum += dt * (this.progress.slow ? 0.45 : 1);
      while (this.accum >= STEP) {
        this.engine.step();
        this.accum -= STEP;
        for (const x of this.engine.events) {
          if (x.type === "delivery") {
            this.progress.delivered++;
            this.progress.best = Math.max(this.progress.best, x.chain ?? 0);
            observed = true;
            this.audio.effect("delivery");
            for (const [i, at] of [5, 20].entries())
              if (
                this.progress.delivered >= at &&
                !this.progress.ownedRewardIds.includes(
                  CONTRAPTION_REWARD_IDS[i],
                )
              ) {
                this.progress.ownedRewardIds.push(CONTRAPTION_REWARD_IDS[i]);
                this.services.awardReward(CONTRAPTION_REWARD_IDS[i]);
              }
            this.schedule();
          } else if (x.type === "touch" || x.type === "switch")
            this.audio.effect(
              x.type === "switch" ? "switch" : (x.part ?? "place"),
            );
          else if (
            x.type === "clear" &&
            !this.progress.clearedIds.includes(this.engineLevel().id)
    ...
    this.services.creditActivePlay(this.activity.step(dt, live, observed));
    this.sync();
    this.paint();
    this.raf = requestAnimationFrame(this.frame);
  };
  ...
  private picture() {
    ...
    out.width = 1650;
    out.height = 990;
    this.renderer.draw(out, this.engine, null, null, this.progress.trails);
    out.toBlob((blob) => {
      if (!blob || this.disposed) return;
      this.imageUrl = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = this.imageUrl;
      a.download = "my-popcorn-machine.png";
      a.click();
      window.setTimeout(() => {
        if (this.imageUrl) {
  ...
  setPaused(p: boolean) {
    this.paused = p;
    this.accum = 0;
    if (p) this.activity.suspend();
    this.sync();
  }
  setMuted(m: boolean) {
    this.progress.mute = m;
