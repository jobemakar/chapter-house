import type { RoomDefinition, Vec, Viewport } from "./types";

export const LANDSCAPE_VIEWPORT: Viewport = Object.freeze({ width: 800, height: 560 });
export const PORTRAIT_VIEWPORT: Viewport = Object.freeze({ width: 560, height: 800 });
...
function portraitPoint(point: Vec): Vec {
  return { x: 280 + (point.x - 400) * 0.68, y: 90 + (point.y - 50) * 1.35 };
}

export const PORTRAIT_ROOMS: readonly RoomDefinition[] = ROOMS.map((room) => {
  const keyStart = portraitPoint(room.keyStart);
  return {
    ...room,
    keyStart,
    cords: room.cords.map((cord) => {
      const anchor = portraitPoint(cord.anchor);
      return { ...cord, anchor, length: Math.hypot(anchor.x - keyStart.x, anchor.y - keyStart.y) };
    }),
    tickets: room.tickets.map((ticket) => ({ ...ticket, position: portraitPoint(ticket.position) })),
    goal: portraitPoint(room.goal),
    props: room.props.map((prop) => ({ ...prop, position: portraitPoint(prop.position), radius: prop.radius * 0.85 })),
  };
});

export function roomsForLayout(portrait: boolean): readonly RoomDefinition[] { return portrait ? PORTRAIT_ROOMS : ROOMS; }
