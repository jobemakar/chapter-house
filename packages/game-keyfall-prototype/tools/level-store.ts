...
export function createLevelStorePlugin(options: LevelStoreOptions = {}): Plugin {
  ...
  return {
    name: "keyfall-local-level-store",
    apply: "serve",
...
async function handleRequest(
  ...
  try {
    ...
    if (request.method === "PUT" && pathname === `${API_ROOT}/index`) {
      ...
      await validateCampaignFiles(root, index);
      await atomicWrite(root, "index.json", index);
      sendJson(response, 200, { ok: true });
      return;
    }
    const match = /^\/api\/keyfall\/levels\/([^/]+)$/.exec(pathname);
    if (request.method === "PUT" && match) {
      requireJson(request);
      const id = decodeLevelId(match[1]);
      const file = parseClientValue(await readJsonBody(request), parseLevelFile);
      if (file.room.id !== id) throw new HttpError(400, `URL id ${id} does not match document id ${file.room.id}`);
      await atomicWrite(root, `${id}.json`, file);
      sendJson(response, 200, { ok: true });
      return;
    }
    throw new HttpError(405, "Method or Keyfall API route is not supported");
  } catch (error) {
    const status = error instanceof HttpError ? error.status : error instanceof SyntaxError ? 400 : 500;
    const message = error instanceof Error ? error.message : "Unexpected level store error";
...
async function listLevels(root: string): Promise<{ levels: LevelFile[]; errors: string[] }> {
  ...
  for (const filename of filenames) {
    ...
    try {
      ...
      if (file.room.id !== id) throw new Error(`document id ${file.room.id} does not match filename`);
      levels.push(file);
    } catch (error) {
...
async function validateCampaignFiles(root: string, index: LevelIndex): Promise<void> {
  for (const id of index.levels) {
    ...
    if (file.room.id !== id) throw new HttpError(400, `Cannot include ${id}: document id is ${file.room.id}`);
...
function validateLocalRequest(request: IncomingMessage, requireOrigin: boolean): void {
  ...
  if (hostname !== "127.0.0.1" && hostname !== "localhost" && hostname !== "[::1]") {
    throw new HttpError(403, "Keyfall level storage is available only on the local host");
  }
